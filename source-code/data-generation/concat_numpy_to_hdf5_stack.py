...
def main():
    ...
    with h5py.File(args.hdf5_file, 'w') as h5_file:
        dataset = h5_file.create_dataset(args.dataset, shape=shape[:2] + (0,), maxshape=shape[:2] + (None,), dtype=dtype)

        # Append the remaining arrays
        for file_name in file_names:
            with open(file_name, 'rb') as np_file:
                dataset.resize(shape[:2] + (dataset.shape[-1] + 4,))
                dataset[:, :, -4:] = np.load(np_file)
